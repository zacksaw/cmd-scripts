...
CountTable <- read.table("quant.featurecounts.counts.unstr.txt", header=TRUE, row.names=1) 
#remove extra columns from feature counts output
...
plotMA(DatasetProcessed, main="DESeq2", ylim=c(-25,25))

############## INDIVIDUAL CONTRAST ANALYSES ############## 

# Set CtrlBU dataset as baseline
...
# Pulling the gene labels from ensemble
...
### VOLCANO PLOTS
Dataset$condition <- relevel(Dataset$condition, "SUDEP")
res <- lfcShrink(DatasetProcessed, contrast=c("condition","Dravet","SUDEP"))
